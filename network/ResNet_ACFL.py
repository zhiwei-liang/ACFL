...
class ResNet(nn.Module):
    def __init__(self, block, layers, num_classes=1000, backbone=False, feature_norm=False):
        ...
        # projection head(new)
        self.projection_head = ProjectionHead(input_dim=512 * block.expansion, hidden_dim=4096, output_dim=2048)
    ...
    def forward(self, x, task=None):
        ...
        elif task == "get_representations":
            representations = self.projection_head(x)
            return representations
        elif task == 'main':
            representations = self.projection_head(x)
            return self.fc_class(x), representations
        else:
            x = self.fc_class(x)
            return x
...
class MultiLayerMLPDiscriminator(nn.Module):
    def __init__(self, input_dim, output_dim, hidden_dim=[512, 256, 128], num_classes = 6):
        super(MultiLayerMLPDiscriminator, self).__init__()
        layers = []
        in_dim = input_dim
